...
- Headless automation summary: add `--run-summary <path>` to `scripts/verdict <evidence>` to write a machine-readable pointer/QA JSON with run IDs, artifact paths, report QA, release-gate/expert-signoff state, readiness state, blockers, and warnings; `scripts/verdict` delegates to the internal `find-evil-auto` engine.
