...
- EID 1102 (Security log cleared) is not automatically incident anti-forensics. A clear under a **template/default hostname** (e.g. `WIN10-TEST`, `WINDOWS2012R2`, `MICROSO-*`) by the **local Administrator** at image-build time is range/golden-image build residue; only a clear by a **domain account** at incident time on the deployed (FQDN, domain-joined) host is incident-relevant. Read the `<Computer>` name, the timestamp, and the actor from `UserData/LogFileCleared/SubjectUserName` before classifying. Several "host" images in a corpus may be clones of one template (identical build-time clear) — do not count them as N separate clearings.
