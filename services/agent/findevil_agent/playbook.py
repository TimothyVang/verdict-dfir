...
TOOL_SEQUENCES: dict[str, list[PlaybookStep]] = {
    "disk": [
        ...
        PlaybookStep("mft_timeline", "MFT timeline with timestomp detection ($SI vs $FN)", pool="both"),
        ...
        PlaybookStep("evtx_query", "Security/System/Application EVTX (EID 4624/4625/4688/7045)", pool="A"),
        PlaybookStep("hayabusa_scan", "Sigma rules over EVTX — persistence + lateral movement", pool="A"),
        PlaybookStep("prefetch_parse", "Prefetch execution evidence (cross-ref with memory PIDs)", pool="A"),
    ...
    "memory": [
        ...
        PlaybookStep("vol_psscan", "EPROCESS pool scan — finds DKOM-unlinked processes", pool="both"),
        PlaybookStep("vol_psxview", "Cross-view comparison when pslist/psscan diverge", pool="both"),
    ...
    "evtx": [
        ...
        PlaybookStep("hayabusa_scan", "Sigma rule scan over EVTX directory", pool="A", optional=True),
    ],
    "network": [
        ...
        PlaybookStep("sysmon_network_query", "Sysmon network events (EID 3/22)", pool="B", optional=True),
    ],
    "velociraptor": [
    ...
    "directory": [
        PlaybookStep("disk_extract_artifacts", "Classify and route artifacts in case directory", pool="both"),
    ],
    "unknown": [],
}
...
def classify_artifact_path(path: str) -> dict[str, str | None]:
    ...
    if name.endswith(".zip"):
        return {"artifact_class": "velociraptor", "evidence_type": "velociraptor", "parser_tool": "vel_collect"}
