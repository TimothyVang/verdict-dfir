...
class TestCorrelateFindings:
    ...
    async def test_tool_classes_overrides_prose_and_downgrades(self) -> None:
        # The prose claims "Prefetch + Amcache" (which the legacy heuristic would
        # KEEP), but the structural tool_classes map shows the finding cites only a
        # registry-class tool — one class — so the structural rule downgrades it.
        result = await CORRELATE_SPEC.handler(
            CorrelateFindingsInput(
                findings=[
                    _finding(
                        description="Prefetch and Amcache both show the binary executed",
                        mitre_technique="T1059.001",
                        confidence="CONFIRMED",
                        tool_call_id="tc-A",
                    )
                ],
                tool_classes={"tc-A": "registry"},
            )
        )
        assert result.outcomes[0].action == "downgraded"
        assert result.refined[0]["confidence"] == "INFERRED"

    async def test_tool_classes_keeps_two_class_execution(self) -> None:
        result = await CORRELATE_SPEC.handler(
            CorrelateFindingsInput(
                findings=[
                    _finding(
                        description="binary executed on the host",
                        mitre_technique="T1059.001",
                        confidence="CONFIRMED",
                        tool_call_id="tc-A",
                        derived_from=["tc-B"],
