...
For the build/test commands themselves, see [CONTRIBUTING.md](https://github.com/TimothyVang/verdict-dfir/blob/master/CONTRIBUTING.md).
...
| Branch | Role |
|--------|------|
| `master` | The single long-lived line — both the published, release-quality line and the contribution line. All pull requests target `master`. |

A maintainer may keep additional private staging outside this repo; that is invisible to contributors
and never required to contribute. Everything you need is `master`.

## How to contribute

1. **Fork** this repo and clone your fork.
2. **Branch off `master`:** `git switch master && git switch -c fix/<short-topic>`.
...
5. **Open a pull request against `master`** with a clear summary and a test plan.
...
- **Two approving reviews, including a maintainer / code owner** (see [`.github/CODEOWNERS`](https://github.com/TimothyVang/verdict-dfir/blob/master/.github/CODEOWNERS)).
...
- A branch up to date with `master`.
...
1. Integrates `master` and re-validates the gate.
