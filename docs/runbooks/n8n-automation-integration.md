...
| Integration surface | **`n8n-mcp` (MIT), user-scope MCP server** | Claude Code uses `n8n-mcp` to build/validate/deploy n8n workflows and trigger runs. The 57-tool product surface is untouched. |
...
- **Not in the judge-facing required docs.** `docs/architecture.md` (Devpost Required Component
  #3) intentionally does **not** mention n8n — the submission surface is the 57-tool typed
...
- It does not add n8n to the investigation flow, the audit chain, the 57-tool count, or
