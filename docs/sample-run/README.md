...
under `--sift`, proving the CONFIRMED core is identical either way (the local run additionally carries
the newer SAM/MRU/shellbag lanes — recall 36%, up from 7%); the fifth proves the **self-correction
...
| [`nist-hacking-case-sift/`](nist-hacking-case-sift/) | Same `SCHARDT.dd`, run under `--sift` (Prefetch **+** registry/UserAssist inside the SIFT VM over SSH) | **SUSPICIOUS** | 9 (8 CONFIRMED + 1 HYPOTHESIS) | **Mode parity (CONFIRMED core):** the identical 8-CONFIRMED 2-class escalation, driven inside the SANS SIFT VM over SSH instead of on the host — a judge gets the same load-bearing result either way. This is the **earlier baseline** (pre-SAM/MRU/shellbag); the local run above now carries those newer lanes, so a refreshed `--sift` parity run is pending the VM. |
