...
| 4 | Velociraptor `.zip` | `scripts/verdict evidence/<coll>.zip` | zip extracted safely (zip-slip / oversize rejected); per-artifact tools ran; Findings cite `tool_call_id`; manifest verifies | **WORKS** to the extent the 19 typed tools reach the carried artifacts; classes they don't cover are a documented gap, not `NO_EVIL` |
