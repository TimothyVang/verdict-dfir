...
def test_chains_doctor() -> None:
    assert "doctor.sh" in SCRIPT.read_text(encoding="utf-8"), "verdict does not reference doctor.sh"


def test_chains_build() -> None:
    text = SCRIPT.read_text(encoding="utf-8")
    assert "cargo build" in text or "findevil-mcp" in text, "verdict does not reference cargo build / findevil-mcp"


def test_chains_engine() -> None:
    assert "find_evil_auto" in SCRIPT.read_text(encoding="utf-8"), "verdict does not chain the find_evil_auto engine"
...
def test_dry_run_produces_no_investigation() -> None:
    result = subprocess.run(["bash", str(SCRIPT), "--dry-run"], capture_output=True, text=True, timeout=10)
...
def test_dry_run_with_skip_build() -> None:
    result = subprocess.run(["bash", str(SCRIPT), "--dry-run", "--skip-build"], capture_output=True, text=True, timeout=10)
